...
def test_parse_simple_template():
    ...
    attr = StringAttribute(hash_key=True, template="USER#{email}")
...
def test_parse_multiple_placeholders():
    ...
    attr = StringAttribute(range_key=True, template="ORDER#{order_id}#{created_at}")
...
def test_parse_static_template():
    ...
    attr = StringAttribute(range_key=True, template="PROFILE")
...
def test_build_key_multiple_placeholders():
    ...
    attr = StringAttribute(range_key=True, template="ORDER#{order_id}#{date}")
...
def test_model_with_multiple_placeholders():
    ...
    class Order(Model):
        ...
        sk = StringAttribute(range_key=True, template="ORDER#{order_id}#{date}")
...
def test_model_query_with_template():
    ...
    # Query by built pk
    orders = list(Order.sync_query(hash_key="USER#123"))
    assert len(orders) == 2
...
def test_model_query_with_template_placeholder():
    """Test querying using template placeholder instead of hash_key."""
...
@MemoryBackend()
def test_model_query_without_template_requires_hash_key():
    """Test that query without template requires hash_key."""

    class Item(Model):
        model_config = ModelConfig(table="items")
        pk = StringAttribute(hash_key=True)  # No template
        name = StringAttribute()

    Item(pk="ITEM#1", name="Test").sync_save()

    # Should work with hash_key
    items = list(Item.sync_query(hash_key="ITEM#1"))
    assert len(items) == 1

    # Should fail without hash_key (no template to build from)
...
def test_inverted_index_with_template():
    ...
    class UserOrder(Model):
        ...
        inverted_index = GlobalSecondaryIndex(
            index_name="inverted",
            hash_key="sk",  # ORDER#456 becomes pk
            range_key="pk",  # USER#123 becomes sk
        )
...
def test_follow_relationship_inverted_index():
    ...
    class Follow(Model):
        ...
        pk = StringAttribute(hash_key=True, template="FOLLOWER#{follower_id}")
        sk = StringAttribute(range_key=True, template="FOLLOWING#{following_id}")
